...
bool AAlsCharacter::StartMantling(const FAlsMantlingTraceSettings& TraceSettings)
{
	...
	if (MovementBaseUtility::UseRelativeLocation(&MovementBaseData))
	{
		const auto TargetTransform{
			FTransform{TargetRotation, TargetCapsuleLocation}.GetRelativeTransform(TargetPrimitive->GetSocketTransform(Parameters.SocketName))
		};
...
{
	// TODO : Cd_changed : mantle allow to start additional check.
...
void AAlsCharacter::StartRagdollingImplementation()
{
	...
	// TODO : Cd_changed : I changed here to make sure logic start after locomotion mode changed.
...
void AAlsCharacter::StopRagdollingImplementation()
{
	...
	if (!GetAnimationInstance() || GetMesh()->GetComponentSpaceTransforms().IsEmpty())
	{
		return;
	}
	...
	{
		// TODO : Cd_changed : Allow input when ragdoll.
